...
**一台中继带多台客户端**，访客怎么找到目标机器有三种路由方式：

| 方式 | 访客地址 | 说明 |
| --- | --- | --- |
| **路径路由（推荐）** | `https://域名/客户端名/` | 中继自己解析 HTTP 请求行按首段分流（`path_router_port` + `public_base_url`），客户端白名单留空即免独立端口；桌面端勾「路径前缀模式」，dufs 原生 path-prefix 保证页内链接自洽。契约：`路径路由_按首段分流_未知404_离线503` |
| 端口路由 | `https://域名:8081` | 每客户端独立公网端口（白名单分配）。契约：`一台中继两个客户端各走各的端口互不串` |
| 子域路由 | `https://home.域名/` | nginx 按 Host 分流到各客户端的回环端口，每子域一张证书 |

同 id 再上线会顶掉前一个（断线重连语义），id 不要两台机器共用。
